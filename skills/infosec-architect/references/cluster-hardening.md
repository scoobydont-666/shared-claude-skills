...
┌─────────────────────────────────────────────┐
│                 Host Network                 │
│  <internal-subnet>/24 (trusted management)            │
├─────────────────────────────────────────────┤
...
ufw allow from <internal-subnet>/24 to any port 22 proto tcp

# Docker Swarm (manager-to-manager, manager-to-worker)
ufw allow from <internal-subnet>/24 to any port 2377 proto tcp  # Swarm management
ufw allow from <internal-subnet>/24 to any port 7946             # Node discovery (TCP+UDP)
ufw allow from <internal-subnet>/24 to any port 4789 proto udp   # Overlay network
...
iptables -I DOCKER-USER -i eth0 ! -s <internal-subnet>/24 -j DROP
```
