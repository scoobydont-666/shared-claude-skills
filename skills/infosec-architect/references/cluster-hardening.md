...
┌─────────────────────────────────────────────┐
│                 Host Network                 │
│  10.0.1.0/23 (trusted management)       │
├─────────────────────────────────────────────┤
...
ufw allow from 10.0.1.0/23 to any port 22 proto tcp

# Docker Swarm (manager-to-manager, manager-to-worker)
ufw allow from 10.0.1.0/23 to any port 2377 proto tcp  # Swarm management
ufw allow from 10.0.1.0/23 to any port 7946             # Node discovery (TCP+UDP)
ufw allow from 10.0.1.0/23 to any port 4789 proto udp   # Overlay network
...
iptables -I DOCKER-USER -i eth0 ! -s 10.0.1.0/23 -j DROP
```
