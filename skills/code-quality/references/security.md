...
- **Rust:** `Command::new("cmd").arg(input)` — `arg()` prevents injection by design.
  Also: never use `format!()` to build SQL — use query builder or parameterized queries (sqlx, diesel).
...
- 🟡 In Rust: use `secrecy::Secret<T>` to prevent accidental logging/Display of secrets
...
- 🔴 **Rust:** serde deserialization of untrusted input: use `#[serde(deny_unknown_fields)]`,
  set `serde_json::from_reader` with size limits, validate after deserialization.
  Never `mem::transmute` or raw pointer deref on untrusted data.
...
|---|---|---|
| `unsafe` block | Memory safety bypass | Minimize scope, document invariants, wrap in safe API |
| FFI boundary (`extern "C"`) | Null pointers, buffer overflows | Validate all inputs at boundary, use `CStr`/`CString` |
| `mem::transmute` | Type confusion, UB | Avoid entirely; use `From`/`Into` or `bytemuck` |
| Integer overflow | Wrapping in release builds | Use `checked_*` or `saturating_*` methods, or `#[deny(arithmetic_overflow)]` |
| `Send`/`Sync` impl on types with raw ptrs | Data races | Never impl manually unless provably safe; prefer compiler-derived |
| `.unwrap()` on network input | DoS via panic | Always use `?` or explicit error handling |
