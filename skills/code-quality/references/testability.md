...
struct OrderService<R: OrderRepo, N: Notifier> {
    repo: R,
    notifier: N,
}

// Also good: dynamic dispatch when generics are impractical
struct OrderService {
...
|---|---|---|
| `#[cfg(test)] mod tests` | Unit tests co-located with source | Standard Rust convention |
| `tests/` directory | Integration tests (crate as black box) | `tests/api_test.rs` |
| `mockall` crate | Auto-mock traits for unit tests | `#[automock]` on trait definition |
| `proptest` / `quickcheck` | Property-based testing | Serialization roundtrips, invariants |
| `#[should_panic]` | Expected panics | `#[should_panic(expected = "out of range")]` |
| `#[ignore]` | Slow/expensive tests | Run separately with `cargo test -- --ignored` |
| `rstest` | Parameterized tests + fixtures | `#[rstest]` with `#[case]` for table-driven tests |
| `wiremock` | HTTP mock server | Integration tests for API clients |
| `testcontainers` | Real DB in tests | Postgres/Redis in ephemeral Docker containers |
...
- 🔴 **Rust:** `#[cfg(test)]` module missing in files with non-trivial logic
...
- 🟡 **Rust:** Using `mockall` on types you own when a simple test double struct would suffice
