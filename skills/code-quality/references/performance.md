...
- **Rust:** Watch for: unnecessary `.clone()` / `.to_string()` in hot paths,
  `Box` when stack allocation suffices, `Vec` growing without `Vec::with_capacity()`,
  `Arc` where `Rc` suffices (atomic overhead), `String` where `&str` or `Cow<'_, str>`
  would avoid allocation, collecting iterators prematurely (`collect()` then iterate again)

### Rust-specific performance patterns
| Pattern | Problem | Fix |
|---|---|---|
| `.clone()` in loop | Allocation per iteration | Borrow, use references, or `Cow` |
| `String` params | Forces caller to allocate | Accept `&str` or `impl AsRef<str>` |
| `collect()` then re-iterate | Double traversal + allocation | Chain iterators, use `Iterator` adaptors |
| `Arc<Mutex<T>>` everywhere | Atomic + lock overhead | Use channels, or `Rc<RefCell<T>>` in single-threaded |
| `HashMap` without capacity hint | Repeated rehashing | `HashMap::with_capacity(n)` |
| `format!()` in hot path | Heap allocation per call | Use `write!()` to reusable buffer |
| Missing `#[inline]` on small public fns | Cross-crate call overhead | `#[inline]` or `#[inline(always)]` for tiny hot fns |
| `async` for CPU-bound work | Runtime overhead, no benefit | Use `rayon` or `spawn_blocking` |
| `dyn Trait` in hot path | Vtable indirection, no inlining | Use generics (`impl Trait`) for monomorphization |
...
- 🔴 **Rust:** Holding `MutexGuard` across `.await` — causes deadlocks in async runtimes
- 🟡 **Rust:** `Arc<Mutex<Vec<T>>>` when a lock-free structure or channel would suffice
...
| Rust | flamegraph, perf | DHAT, heaptrack | cargo-flamegraph, `cargo bench` (criterion) |
