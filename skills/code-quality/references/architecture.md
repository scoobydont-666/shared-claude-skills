...
|---|---|---|
| Newtype (`struct UserId(u64)`) | Type safety, prevent primitive obsession | Domain identifiers, validated strings, units |
| Typestate (`struct Order<S: State>`) | Compile-time state machine enforcement | Workflows, builders, protocol states |
| `pub(crate)` visibility | Module encapsulation without `pub` leak | Internal APIs that shouldn't be exported |
| Trait objects vs generics | Flexibility vs performance | `dyn Trait` for heterogeneous collections, `impl Trait` for hot paths |
| Workspace crate boundaries | Enforce module boundaries at compile time | Split domain/infra/api into separate crates |
| Error type hierarchy | One error enum per crate, `From` impls for conversion | Crate-level `Error` with `thiserror`, `anyhow` at binary |
| `#[non_exhaustive]` on public enums | Allow adding variants without breaking downstream | Any public enum that may grow |

**Rust-specific flags:**
- 🔴 `pub` on everything — indicates no thought given to API surface
- 🔴 Single massive `lib.rs` with no module hierarchy
- 🟡 Leaking implementation types in public API (e.g., exposing `sqlx::PgPool` in domain layer)
- 🟡 Using `String` for domain concepts instead of newtypes
